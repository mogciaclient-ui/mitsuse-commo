...
type SurveyResponse = { id: string; lineUserId?: string; lineDisplayName?: string; linePictureUrl?: string; purchaseExperience?: string; store?: string; storeGroup?: string; birthDate?: string; message?: string; createdAt: Timestamp | null };
...
export default function LineUsersPage() {
  ...
  </section> : view === "responses" ? <section className={`card ${styles.responsePanel}`}><div className={styles.sectionHead}><div><h2>アンケート回答</h2><p>現在公開中のアンケートへ届いた回答です。</p></div><a href="/survey" target="_blank" rel="noreferrer">公開ページを確認</a></div>{loading ? <p className="dashboard-status">回答データを読み込んでいます…</p> : <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>回答日</th><th>LINEユーザー</th><th>購入経験</th><th>購入店舗</th><th>生年月日</th><th>自由記述</th></tr></thead><tbody>{responses.map(response => <tr key={response.id}><td>{formatDate(response.createdAt)}</td><td>{response.lineDisplayName || "LINEユーザー"}</td><td>{purchaseLabel(response.purchaseExperience)}</td><td>{response.store || "—"}</td><td>{formatBirthDate(response.birthDate)}</td><td className={styles.message}>{response.message || "—"}</td></tr>)}</tbody></table>{!responses.length ? <p className="dashboard-empty">回答はまだありません。</p> : null}</div>}</section> : <SegmentsDashboard responses={responses} users={lineUsers} />}</div>;
}
