...
export function AdminShell({children}:{children:ReactNode}){
 ...
 return <div className="admin-layout"><aside className="sidebar"><Link href="/dashboard" className="brand">いい麺亭</Link><span className="sidebar-shop">LINE運用管理</span><nav className="nav"><Link className={`nav-main ${pathname==="/dashboard"?"active":""}`} href="/dashboard"><Icon name="home"/>ホーム</Link>{groups.map(g=><div className="nav-group" key={g.label}><Link className="nav-main" href={g.href}><Icon name={g.icon}/>{g.label}<span className="nav-up">⌃</span></Link><div className="nav-children">{g.children.map(c=><Link key={c.href} className={pathname===c.href.split("#")[0]&&!c.href.includes("#")?"active":""} href={c.href}>{c.label}</Link>)}</div></div>)}<Link className={`nav-main ${pathname.includes("ai-suggestions")?"active":""}`} href="/line/ai-suggestions"><Icon name="spark"/>AI提案</Link><Link className={`nav-main ${pathname.includes("settings")?"active":""}`} href="/line/settings"><Icon name="settings"/>設定</Link></nav><button type="button" className="logout" onClick={handleLogout}><Icon name="logout"/>ログアウト</button></aside><main className="content">{children}</main></div>
}
