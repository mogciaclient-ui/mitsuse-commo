...
export function AdminShell({children}:{children:ReactNode}){
 ...
 return <div className="admin-layout"><aside className="sidebar"><Link href="/dashboard" className="brand">いいもの三瀬</Link><span className="sidebar-shop">LINE運用管理</span><nav className="nav"><Link className={`nav-main ${pathname==="/dashboard"?"active":""}`} href="/dashboard"><Icon name="home"/>ホーム</Link><Link className={`nav-main ${pathname==="/line/users"?"active":""}`} href="/line/users"><Icon name="users"/>顧客・アンケート</Link><Link className={`nav-main ${pathname.includes("/line/broadcasts")?"active":""}`} href="/line/broadcasts"><Icon name="send"/>配信</Link><Link className={`nav-main ${pathname.includes("settings")?"active":""}`} href="/line/settings"><Icon name="settings"/>設定</Link></nav><button type="button" className="logout" onClick={handleLogout}><Icon name="logout"/>ログアウト</button></aside><main className="content">{children}</main></div>
}
