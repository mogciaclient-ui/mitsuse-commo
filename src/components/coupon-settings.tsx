...
export function CouponSettings() {
  ...
  async function save() {
    ...
    try { const token = await user.getIdToken(); const response = await fetch("/api/line/coupons", { method: "PUT", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ couponId, couponTitle: coupon.title, thankYouMessage: message, enabled }) }); const body = await readJson(response); if (!response.ok) throw new Error(body.error || "クーポン設定を保存できませんでした。"); setResult({ ok: true, text: "アンケート回答特典を保存しました。" }); }
...
async function readJson(response: Response) {
  ...
  if (!text) return {};
  try { return JSON.parse(text); } catch { throw new Error("サーバーでエラーが発生しました。少し待ってから再読み込みしてください。"); }
}
