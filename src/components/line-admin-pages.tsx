...
function Broadcasts({ composer = false }: { composer?: boolean }) {
  const { broadcasts } = useAdminData(true); const [target, setTarget] = useState("all"); const [message, setMessage] = useState(""); const [purpose, setPurpose] = useState(""); const [sending, setSending] = useState(false); const [generating, setGenerating] = useState(false); const [result, setResult] = useState<{ ok: boolean; text: string } | null>(null);
  useEffect(() => { const selected = new URLSearchParams(window.location.search).get("target"); if (selected && (["answered", "unanswered", "birthday-next"].includes(selected) || selected.startsWith("store:"))) setTarget(selected); }, []);
...
function Suggestions() {
  ...
  return <div className={styles.stack}><header className="page-head"><div><h1>AI提案</h1><p>アンケート回答や配信状況の集計をAIが読み取り、傾向と次の施策を提案します。</p></div><button className="primary" disabled={loading} onClick={analyze}>{loading ? "分析しています…" : analysis ? "もう一度分析" : "AIで分析する"}</button></header>{error ? <p className={styles.error}>{error}</p> : null}{!analysis ? <section className={`card ${styles.aiEmpty}`}><strong>最新データをまとめて分析</strong><p>店舗別の回答数と配信人数の集計をもとに、注目点と次に試す施策を提案します。</p></section> : <><section className={`card ${styles.analysisSummary}`}><small>AIによる要約</small><p>{analysis.summary}</p></section><div className={styles.grid}>{analysis.insights.map(item => <article className={`card ${styles.suggestion}`} key={item.title}><small>注目ポイント</small><h2>{item.title}</h2><p>{item.detail}</p></article>)}</div><section className={`card ${styles.panel}`}><h2>次に試す施策</h2><div className={styles.actionList}>{analysis.actions.map((item, index) => <div key={item.title}><b>{index + 1}. {item.title}</b><p>{item.detail}</p></div>)}</div></section></>}</div>;
}
